...
app.post('/stopvm', (req, res) => {
  ...
  const command = `"C:\\Program Files (x86)\\VBox\\VBoxManage.exe" controlvm "${vmName}" poweroff`;

  exec(command, (error, stdout, stderr) => {
    if (error) {
      console.error(`Fehler: ${error.message}`);
      return res.status(500).send(`Fehler beim Stoppen der VM: ${error.message}`);
    }
    if (stderr) {
      console.error(`stderr: ${stderr}`);
      return res.status(500).send(`Fehlerausgabe: ${stderr}`);
    ...
    console.log(`stdout: ${stdout}`);
    res.send(`VM "${vmName}" wurde erfolgreich gestoppt!`);
  });
});
